...
//Подсчет геометрической вероятности для q_j
double geom_probability(double x) {
	return p2 * pow((1 - p2), (x - 1));
}

//Мера расхождения теоретической и выборочной функций распределения
...
int main() {
	...
	for (int i = 0; i < n; i++) {
		...
		double left_b = max1; // Максимальное значение выборки
		...
		cout << "Введите интервалы разбиения" << endl;
		for (int z = 1; z < k; z++) {
			cout << "z" << z << endl;
			cin >> splitting[z];
		}
